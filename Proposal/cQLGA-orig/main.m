% -- Synchronous Cellular Quantum Inspired Genetic Algorithm --------
% Initialisation: biased 1/sqrt(2)
% Interference: rotation gate
% crossover: two-point
% mutation: bit-flip
% selection: binary tournament
% grid: square
% Neighbourhood: Moore
% Population: coded as Betas and not alphas
% additional info: records the gbest (binary and probabilistic + variance of the population)
% Very important: it corrects the update formula of beta in test1, 2 and 3
...
%% -------- Prameters  of the dynamic cellular Genetic Algorithm  ------
   ...
   theta = 1; % The choice is based on mathematical demonstration % BTW I am using a rotation_gate = [cosd(theta) -sind(theta);sind(theta) cosd(theta)];
...
for i=0:(grid_y - 1)
    for j=1:grid_x
        ...
        % We update the beta using the formula: sin(theta)alpha + cos(theta)beta
        parent_processed = (sind(theta)*(sqrt(1 - parent_1.^2))) + (cosd(theta)*parent_1);
        ...
        c1_mutated(c1_mut_pos) = sqrt(1 - c1(c1_mut_pos).^2); 
        c2_mutated(c2_mut_pos) = sqrt(1 - c2(c2_mut_pos).^2);
        %% ------------ Quantum measurement ---------------------------------
        c1_bin = rand(1,Dimension) <= c1_mutated.^2;
        c2_bin = rand(1,Dimension) <= c2_mutated.^2;
